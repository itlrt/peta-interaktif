...
export default function EditStationPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchStation = async () => {
      try {
        ...
        setDestinationPreviews(station.destinations.map(dest => dest.imageUrl || ""))
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Gagal memperbarui stasiun")
      }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-gray-800">Edit Stasiun</h1>
        </div>
        
        <div className="flex gap-3">
          <Link
            href="/admin/stations"
            className="px-4 py-2 border border-gray-300 rounded-md text-gray-700 text-sm font-medium hover:bg-gray-50 transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
          >
            Batal
          </Link>
          <button
      ...
      {error && (
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4">
          ...
          <span className="block sm:inline"> Terjadi kesalahan saat mengedit stasiun. Mohon coba lagi.</span>
        </div>
      )}

      <form id="station-form" onSubmit={handleSubmit} className="space-y-8">
              ...
              {destinations.map((destination, index) => (
                <div key={index} className="p-4 border rounded-md bg-gray-50">
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      {(destinationPreviews[index] || destination.imageUrl) && (
                        <div className="relative w-full h-32 mb-2">
                          <Image
                            src={destinationPreviews[index] || destination.imageUrl || ""}
                            alt="Preview"
                            fill
                            className="object-cover rounded-lg"
                          />
